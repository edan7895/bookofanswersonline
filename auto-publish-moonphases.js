...
// 自动发布 moon-phases 文章 + index.html

const fs = require("fs");
const path = require("path");
const { execSync } = require("child_process");

const scheduledDir = "articles/scheduled/moon-phases";
const targetDir = "articles/moon-phases";
...
if (files.length === 0) {
  console.log("📭 今天没有要发布的 moon-phases 文件。");
  process.exit(0);
}
...
try {
  ...
  execSync(`git commit -m "🌙 Auto publish moon-phases article & index for ${today}"`);
  execSync(`git push`);
  console.log("🎉 moon-phases 已成功发布！");
} catch (err) {
